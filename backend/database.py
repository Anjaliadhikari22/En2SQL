...
# Cache engines and connection status per dialect
...
def get_engine(db_type: str) -> Engine:
    """Return (and cache) a SQLAlchemy engine for the given database type."""
    db_type = _normalize_db_type(db_type)
    if db_type not in _engines:
        uri = Config.get_sqlalchemy_uri(db_type)
...
def is_db_connected(db_type: str) -> bool:
    """
    ...
    """
    db_type = _normalize_db_type(db_type)
    if db_type in _connection_cache:
        return _connection_cache[db_type]

    try:
        engine = get_engine(db_type)
        with engine.connect() as conn:
            conn.execute(text("SELECT 1"))
        _connection_cache[db_type] = True
    except Exception:
        _connection_cache[db_type] = False

    return _connection_cache[db_type]


def get_session_factory(db_type: str) -> sessionmaker:
    """Create a session factory bound to the engine for db_type."""
    return sessionmaker(bind=get_engine(db_type), autocommit=False, autoflush=False)


@contextmanager
def get_session(db_type: str) -> Generator[Session, None, None]:
    """Context manager that yields a SQLAlchemy session and closes it safely."""
    SessionLocal = get_session_factory(db_type)
...
def test_connection(db_type: str) -> dict[str, Any]:
    """Ping the database to verify connectivity."""
    db_type = _normalize_db_type(db_type)
    connected = is_db_connected(db_type)
    return {
        "connected": connected,
        "db_type": db_type,
